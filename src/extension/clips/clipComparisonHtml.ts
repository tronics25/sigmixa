...
export function clipComparisonHtml(webview: vscode.Webview, extensionUri: vscode.Uri, locale: string): string {
  const script = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'out', 'webview', 'clipComparison.js')); const codicons = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'codicons', 'codicon.css')); const token = nonce();
  return `<!doctype html><html lang="${locale.toLowerCase().startsWith('ja') ? 'ja' : 'en'}"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src ${webview.cspSource} 'unsafe-inline'; font-src ${webview.cspSource}; script-src 'nonce-${token}';"><link rel="stylesheet" href="${codicons}"><title>Clip Comparison</title></head><body><main id="app"></main><script nonce="${token}" src="${script}"></script></body></html>`;
}
