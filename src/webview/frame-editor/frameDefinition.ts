...
function derivedSummary(signal: ManualDerivedSignalDefinition): string {
  ...
  if (signal.operation.type === 'lookup') return `${signal.operation.input} · Linear · ${signal.operation.points.length} points · ${signal.operation.outOfRange}`;
...
.definition-stack{display:flex;flex-direction:column;gap:5px}.definition-controls{display:grid;grid-template-columns:minmax(140px,1fr) minmax(110px,.6fr);gap:5px}.lookup-table{display:grid;grid-template-columns:minmax(100px,1fr) minmax(100px,1fr) 30px;gap:3px;align-items:center}.lookup-head{font-size:10px;font-weight:650;color:var(--vscode-descriptionForeground);padding:2px 5px}.lookup-table .icon{height:27px}.definition-stack>.secondary{align-self:flex-start}
...
</style><div id="content"><p class="muted">Loading frame definition…</p></div>`;
...
function updateFrame(patch: Partial<ManualFrameDefinition>): void { if (frame) frame = { ...frame, ...patch }; }
function updateSignal(id: string, transform: (signal: ManualSignalDefinition) => ManualSignalDefinition): void {
  if (frame) frame = { ...frame, signals: frame.signals.map((signal) => signal.id === id ? transform(signal) : signal) };
}
function updateDerivedSignal(id: string, transform: (signal: ManualDerivedSignalDefinition) => ManualDerivedSignalDefinition): void {
  if (frame) frame = { ...frame, derivedSignals: (frame.derivedSignals ?? []).map((signal) => signal.id === id ? transform(signal) : signal) };
}
...
function inputField(label: string, value: string, onChange: (value: string) => void, options: { type?: string; width?: string; disabled?: boolean } = {}): HTMLElement {
  ...
  input.addEventListener('change', () => { onChange(input.value); serverDiagnostics = []; saveStatus = ''; render(); });
...
function render(): void {
  ...
  const title = document.createElement('h1'); title.textContent = 'Frame Definition';
  ...
  fields.append(
    ...
    inputField('Frame name', frame.name, (value) => updateFrame({ name: value }), { width: '240px' })
  );
  const lengthField = document.createElement('label'); lengthField.className = 'field'; lengthField.innerHTML = '<span>Frame length</span>';
  ...
  const bitTitle = document.createElement('h2'); bitTitle.textContent = 'Bit Layout'; content.append(bitTitle, buildBitLayout(frame));
  const signalTitle = document.createElement('h2'); signalTitle.textContent = 'Signal Definitions'; content.appendChild(signalTitle);
  const toolbar = document.createElement('div'); toolbar.className = 'toolbar'; toolbar.style.margin = '12px 0 7px';
  const add = button('+ Add Signal', () => { const previous = frame?.signals[frame.signals.length - 1]; if (frame) updateFrame({ signals: [...frame.signals, createManualSignal(uuid(), previous, frame.frameLength)] }); render(); }, true);
  const autoFit = button('Auto Fit', () => { if (!frame) return; widths = { ...widths, ...autoFitColumns(columnDefinitions, frame.signals, measureText) }; persist(); render(); });
  const save = button('Save Definition', () => saveFrame(), true);
  const spacer = document.createElement('span'); spacer.className = 'spacer';
  const status = document.createElement('span'); status.className = saveStatus.startsWith('Saved') ? 'success' : 'muted'; status.textContent = saveStatus;
  toolbar.append(add, autoFit, spacer, status, save); content.append(toolbar, buildSignalTable(frame));
  const derivedTitle = document.createElement('h2'); derivedTitle.textContent = 'Derived Signal Definitions'; content.appendChild(derivedTitle);
  const derivedToolbar = document.createElement('div'); derivedToolbar.className = 'toolbar'; derivedToolbar.style.margin = '12px 0 7px';
  const addDerived = button('+ Add Derived Signal', () => { if (frame) updateFrame({ derivedSignals: [...(frame.derivedSignals ?? []), createManualDerivedSignal(uuid())] }); render(); }, true);
  const autoFitDerived = button('Auto Fit', () => { if (!frame) return; widths = { ...widths, ...autoFitColumns(derivedColumnDefinitions, frame.derivedSignals ?? [], measureText) }; persist(); render(); });
  const derivedNote = document.createElement('span'); derivedNote.className = 'muted'; derivedNote.textContent = 'Definitions may reference extracted Signals and only the Derived Signals above the current row.';
  ...
  const note = document.createElement('p'); note.className = 'muted'; note.textContent = 'Signal Definitions always apply Scale + Offset after bit extraction. Changing byte, bit, or length never changes byte order; a new Signal inherits the previous byte order.'; content.appendChild(note);
}

function buildBitLayout(value: ManualFrameDefinition): HTMLElement {
  const owner = new Map<number, number[]>();
  value.signals.forEach((signal, signalIndex) => { for (const bit of occupiedBits(signal)) { const entries = owner.get(bit) ?? []; entries.push(signalIndex); owner.set(bit, entries); } });
  const wrap = document.createElement('div'); wrap.className = 'bit-layout'; const grid = document.createElement('div'); grid.className = 'byte-grid';
...
function buildSignalTable(value: ManualFrameDefinition): HTMLElement {
  ...
  if (!value.signals.length) { const empty = document.createElement('p'); empty.className = 'muted'; empty.style.padding = '8px'; empty.textContent = 'No manual signals are defined.'; wrap.appendChild(empty); }
  return wrap;
}

function buildDerivedTable(value: ManualFrameDefinition): HTMLElement {
  const derived = value.derivedSignals ?? []; const wrap = document.createElement('div'); wrap.className = 'table-wrap'; const header = document.createElement('div'); header.className = 'derived-header'; header.style.gridTemplateColumns = template(derivedColumnDefinitions);
  ...
  derived.forEach((signal, index) => {
    ...
    row.append(
      ...
      cell(selectInput([['expression','Expression'],['lookup','Lookup Table'],['filter','Filter']], signal.operation.type, (next) => updateDerivedSignal(signal.id, (current) => ({ ...current, operation: operationForType(next as ManualDerivedSignalDefinition['operation']['type'], current.operation, available) })))),
  ...
  if (!derived.length) { const empty = document.createElement('p'); empty.className = 'muted'; empty.style.padding = '8px'; empty.textContent = 'No Derived Signals are defined.'; wrap.appendChild(empty); }
  return wrap;
}
...
function signalNameSelect(names: readonly string[], value: string, change: (value: string) => void): HTMLSelectElement {
  const options: Array<readonly [string, string]> = [['', 'Select input Signal…'], ...names.map((name) => [name, name] as const)];
...
function lookupInput(signal: ManualDerivedSignalDefinition, available: readonly string[]): HTMLElement {
  ...
  controls.append(
    ...
    selectInput([['clamp','Clamp outside range'],['error','Error outside range']], operation.outOfRange, (next) => updateDerivedSignal(signal.id, (current) => ({ ...current, operation: current.operation.type === 'lookup' ? { ...current.operation, outOfRange: next as 'clamp' | 'error' } : current.operation })))
  );
...
function filterInput(signal: ManualDerivedSignalDefinition, available: readonly string[]): HTMLElement {
  ...
  controls.append(
    ...
    selectInput([['low-pass','Low-pass'],['moving-average','Moving average']], operation.filter, (next) => updateDerivedSignal(signal.id, (current) => ({ ...current, operation: current.operation.type === 'filter' ? { ...current.operation, filter: next as 'low-pass' | 'moving-average' } : current.operation })))
  );
...
function buildDiagnostics(items: readonly Diagnostic[]): HTMLElement { const box = document.createElement('div'); box.className = 'diagnostics'; const strong = document.createElement('strong'); strong.textContent = `Definition has ${items.length} issue${items.length === 1 ? '' : 's'}`; const list = document.createElement('ul'); for (const item of items.slice(0, 20)) { const row = document.createElement('li'); row.textContent = item.message; list.appendChild(row); } box.append(strong,list); return box; }
function saveFrame(): void { if (!frame) return; const validation = validateFrameDefinition(frame); serverDiagnostics = validation.diagnostics; if (!validation.valid) { saveStatus = 'Fix validation issues before saving.'; render(); return; } saveStatus = 'Saving…'; render(); vscode.postMessage({ type: 'save', frame }); }

window.addEventListener('message', (event: MessageEvent<FrameEditorToWebview>) => { const message = event.data; if (message.type === 'init') { frame = structuredClone(message.frame); if (message.widths) widths = { ...widths, ...message.widths }; serverDiagnostics = []; saveStatus = ''; render(); } else if (message.type === 'saveResult') { serverDiagnostics = message.diagnostics; if (message.saved && message.frame) { frame = structuredClone(message.frame); saveStatus = 'Saved.'; } else saveStatus = 'Save failed.'; render(); } });
