...
import { bindDecodedInspection, hideDecodedInspection } from './decodedInspection';
...
<style>
  *{box-sizing:border-box} body{padding:0;margin:0;color:var(--vscode-foreground);background:var(--vscode-editor-background);font-family:var(--vscode-font-family)}
  ...
  .toolbar-secondary{display:flex;align-items:center;gap:2px;padding-left:6px;border-left:1px solid var(--vscode-panel-border)}button.icon-button{display:inline-flex;width:28px;height:28px;min-width:28px;align-items:center;justify-content:center;padding:0;border-color:transparent;background:transparent;color:var(--vscode-icon-foreground,var(--vscode-foreground))}button.icon-button:hover:not(:disabled){background:var(--vscode-toolbar-hoverBackground,var(--vscode-list-hoverBackground))}button.icon-button.primary{color:var(--vscode-focusBorder);background:color-mix(in srgb,var(--vscode-focusBorder) 13%,transparent)}button.icon-button:disabled{opacity:.34}button.icon-button .codicon{font-size:16px}.chart-navigation button.icon-button{margin-left:auto}.trajectory-camera-controls.segmented{gap:0}.trajectory-camera-controls.segmented button{border-radius:0}.trajectory-camera-controls.segmented button:first-child{border-radius:3px 0 0 3px}.trajectory-camera-controls.segmented button:last-child{border-radius:0 3px 3px 0}
  ...
  .signal-change-numeric{background:color-mix(in srgb,var(--vscode-charts-yellow,#d7ba7d) var(--signal-change-strength),transparent)}
  .signal-change-state{background:color-mix(in srgb,var(--vscode-charts-purple,#b48ead) 22%,transparent);box-shadow:inset 2px 0 color-mix(in srgb,var(--vscode-charts-purple,#b48ead) 65%,transparent)}
  .signal-change-recovery{position:relative}.signal-change-recovery::after{content:"";position:absolute;top:3px;right:3px;width:4px;height:4px;border-radius:50%;background:var(--vscode-charts-blue,#69b8f0);pointer-events:none}
</style>
...
function appendContent(cell: HTMLElement, row: RawRowDto): void {
  ...
  else for (const [index, tag] of row.decodedContent.split('  ·  ').entries()) { const chip = document.createElement('span'); chip.className = 'tag'; chip.textContent = tag; const detail = row.decodedSignals?.[index]; if (detail && detail.tag === tag) bindDecodedInspection(chip, row, detail); line.appendChild(chip); }
