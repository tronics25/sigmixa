...
import { crossesSeriesGap, displaySeriesRange, renderedSeriesValueAt } from '../shared/chartSeries';
...
import { layoutHoverLabelBoxes, layoutTimestampMarker, type HoverLabelRect } from '../shared/chartHoverLabels';
...
interface CanvasHoverValue { readonly y: number; readonly text: string; readonly color: string; }
interface CanvasTimestampMarker { readonly text: string; readonly rect: HoverLabelRect; readonly emphasized?: boolean; }
...
export class TimeSeriesView {
  ...
  constructor(
    ...
    for (const [selector, label] of [['.zoom-in', t('Zoom in', '拡大')], ['.zoom-out', t('Zoom out', '縮小')]] as const) { const button = host.querySelector(selector)!; button.setAttribute('title', label); button.setAttribute('aria-label', label); }
    host.querySelector('.zoom-in')!.addEventListener('click', () => this.zoom(1 / 1.5)); host.querySelector('.zoom-out')!.addEventListener('click', () => this.zoom(1.5)); host.querySelector('.zoom-reset')!.addEventListener('click', () => this.requestSeries(this.fullRange));
    ...
    host.querySelector('.clip-clear')!.addEventListener('click', () => { this.selectedRange = undefined; this.selectionAnchor = undefined; this.selectionGraphIndex = undefined; this.rangeSignalIds = []; this.activeRangeEdge = undefined; this.rangeDragged = false; this.updateClipControls(); this.draw(false); });
    ...
    host.querySelector('.clip-create')!.addEventListener('click', () => { if (this.selectedRange) this.post({ type: 'createClip', startTimestamp: this.selectedRange.start, endTimestamp: this.selectedRange.end, signalIds: [...this.selected] }); });
    ...
    host.querySelector('.clip-start-prev')!.addEventListener('click', () => this.adjustRange('start', -1)); host.querySelector('.clip-start-next')!.addEventListener('click', () => this.adjustRange('start', 1)); host.querySelector('.clip-end-prev')!.addEventListener('click', () => this.adjustRange('end', -1)); host.querySelector('.clip-end-next')!.addEventListener('click', () => this.adjustRange('end', 1));
    ...
    this.applyPane(); this.updateModeButtons(); this.updateClipControls(); this.updateMarkerControls(); this.updateStatus();
  ...
  handle(message: ToWebviewMessage): void {
    if (message.type === 'externalCsvPreview') {
      ...
      this.selectedRange = undefined; this.selectionAnchor = undefined; this.selectionGraphIndex = undefined; this.rangeSignalIds = []; this.activeRangeEdge = undefined; this.rangeDragged = false; this.updateClipControls(); this.draw(false);
    } else if (message.type === 'timeRangeAdjusted' && message.requestId === this.latestRangeRequest) {
      this.selectedRange = { start: message.startTimestamp, end: message.endTimestamp }; const edge = this.activeRangeEdge; if (edge) this.cursorTimestamp = this.selectedRange[edge]; this.hoverGraphIndex = findGraphIndexBySignals(timeSeriesGraphSignalIds(timeSeriesGraphs(this.series, this.mode, this.groupLayout)), this.rangeSignalIds); this.updateClipControls(); this.draw(false);
  ...
  private selectionChanged(): void { this.series = []; this.persist(); this.updateClipControls(); this.requestSeries(); }
  ...
  private beginRangeSelection(event: MouseEvent): void {
    ...
    const rect = this.canvas.getBoundingClientRect(); const graphIndex = graphIndexAtOffset(event.clientY - rect.top - 16, this.legendHeights); if (graphIndex === undefined) return;
    ...
    this.selectionAnchor = timestamp; this.selectionAnchorClientX = event.clientX; this.selectionGraphIndex = graphIndex; this.rangeSignalIds = signalIds; this.activeRangeEdge = 'end'; this.rangeDragged = false; this.selectedRange = { start: timestamp, end: timestamp }; this.cursorTimestamp = timestamp; this.hoverGraphIndex = graphIndex; this.timeline?.set(timestamp, 'timeSeries'); event.preventDefault(); this.updateClipControls(); this.draw(false);
  }
  ...
  private endRangeSelection(event: MouseEvent): void {
    ...
    else if (this.selectedRange) { this.latestRangeRequest = ++this.counter; this.post({ type: 'snapTimeRangeRequest', requestId: this.latestRangeRequest, startTimestamp: this.selectedRange.start, endTimestamp: this.selectedRange.end }); }
    this.selectionGraphIndex = undefined;
    this.updateClipControls(); this.draw();
  }
  private updateClipControls(): void {
    const range = this.selectedRange; const valid = Boolean(range && range.end > range.start && this.selected.size);
    const controls = this.host.querySelector<HTMLElement>('.clip-controls')!; controls.hidden = !range || this.selectionAnchor !== undefined;
    (this.host.querySelector('.clip-create') as HTMLButtonElement).disabled = !valid;
    if (range) this.host.querySelector('.clip-range')!.textContent = `${t('Start', '開始')} ${formatNumber(range.start)} s · ${t('End', '終了')} ${formatNumber(range.end)} s · ${t('Duration', '長さ')} ${formatNumber(range.end - range.start)} s`;
  }

  private adjustRange(edge: 'start' | 'end', direction: -1 | 1): void { if (!this.selectedRange) return; this.activeRangeEdge = edge; this.cursorTimestamp = this.selectedRange[edge]; this.hoverGraphIndex = findGraphIndexBySignals(timeSeriesGraphSignalIds(timeSeriesGraphs(this.series, this.mode, this.groupLayout)), this.rangeSignalIds); this.draw(false); this.latestRangeRequest = ++this.counter; this.post({ type: 'adjustTimeRangeRequest', requestId: this.latestRangeRequest, edge, direction, startTimestamp: this.selectedRange.start, endTimestamp: this.selectedRange.end }); }
  ...
  private paintChart(context: CanvasRenderingContext2D, width: number, height: number, top: number, bottom: number, interactions: boolean, legendHeights: readonly number[]): ChartGeometry | undefined {
    ...
    for (let graphIndex = 0; graphIndex < graphCount; graphIndex++) {
      ...
      pair.forEach((group, axisIndex) => {
        ...
        for (const item of group.items) { const color = this.color(item.definition.id); context.strokeStyle = color; context.lineWidth = 1.5; context.beginPath(); let started = false; let previousTimestamp: number | undefined; for (const sample of item.samples) { const value = seriesDisplayValue(item, sample.value, this.mode, group.displayUnit); const usable = Number.isFinite(value) && (sample.quality === undefined || sample.quality === 'valid'); if (!usable) { if (!this.connectGaps) started = false; continue; } const separated = !this.connectGaps && previousTimestamp !== undefined && crossesSeriesGap(item.gaps, previousTimestamp, sample.timestamp); const px = x(sample.timestamp); const py = y(value); started && !separated ? context.lineTo(px, py) : context.moveTo(px, py); started = true; previousTimestamp = sample.timestamp; } context.stroke(); this.drawEvents(context, item, x, y, group.displayUnit); }
        context.restore();
        if (interactions && this.cursorTimestamp !== undefined && this.hoverGraphIndex === graphIndex) for (const item of group.items) {
          const rawValue = renderedSeriesValueAt(item, this.cursorTimestamp, this.connectGaps); if (rawValue === undefined) continue;
          const value = seriesDisplayValue(item, rawValue, this.mode, group.displayUnit); const py = y(value);
          if (Number.isFinite(py) && py >= laneTop && py <= laneTop + laneHeight) hoverValues.push({ y: py, text: displayValueLabel(value, this.mode, group.displayUnit), color: this.color(item.definition.id) });
        }
        for (const marker of this.markers.values()) {
          if (findGraphIndexBySignals(graphSignalIds, marker.signalIds) !== graphIndex) continue;
          for (const item of group.items) {
          const rawValue = renderedSeriesValueAt(item, marker.timestamp, this.connectGaps); if (rawValue === undefined) continue;
          const value = seriesDisplayValue(item, rawValue, this.mode, group.displayUnit); const py = y(value); if (!Number.isFinite(py) || py < laneTop || py > laneTop + laneHeight) continue;
          const values = markerValues.get(marker.id) ?? []; values.push({ y: py, text: displayValueLabel(value, this.mode, group.displayUnit), color: this.color(item.definition.id) }); markerValues.set(marker.id, values);
          }
        }
      });
      ...
      const visibleMarkers = [...this.markers.values()].filter((marker) => findGraphIndexBySignals(graphSignalIds, marker.signalIds) === graphIndex && marker.timestamp >= this.viewRange!.start && marker.timestamp <= this.viewRange!.end);
      ...
      if (interactions && this.cursorTimestamp !== undefined && this.cursorTimestamp >= this.viewRange.start && this.cursorTimestamp <= this.viewRange.end && (this.hoverGraphIndex === undefined || this.hoverGraphIndex === graphIndex)) { context.strokeStyle = css('--vscode-editorCursor-foreground', '#fff'); context.beginPath(); const px = x(this.cursorTimestamp); context.moveTo(px, laneTop); context.lineTo(px, laneTop + laneHeight); context.stroke(); }
      ...
      const cursorMatchesRangeEdge = this.selectedRange && this.activeRangeEdge && this.cursorTimestamp === this.selectedRange[this.activeRangeEdge] && rangeGraphIndex === graphIndex;
      const timestampMarker = interactions && this.cursorTimestamp !== undefined && this.hoverGraphIndex === graphIndex && !cursorMatchesRangeEdge
        ? this.createTimestampMarker(context, x(this.cursorTimestamp), `${formatNumber(this.cursorTimestamp)} s`, left, left + plotW, laneTop + laneHeight, occupiedLabels, laneTop)
        : undefined;
      ...
      if (interactions && this.cursorTimestamp !== undefined && this.hoverGraphIndex === graphIndex) this.paintHoverValues(context, x(this.cursorTimestamp), hoverValues, left, left + plotW, laneTop, laneTop + laneHeight, occupiedLabels);
  ...
  private paintHoverValues(context: CanvasRenderingContext2D, cursorX: number, values: readonly CanvasHoverValue[], left: number, right: number, top: number, bottom: number, occupied: HoverLabelRect[] = []): void {
    ...
    for (const value of positioned) {
      context.strokeStyle = value.color; context.globalAlpha = .7; context.beginPath(); context.arc(cursorX, value.y, 3, 0, Math.PI * 2); context.fillStyle = value.color; context.fill(); if (Math.abs(value.labelY - value.y) > 1 || (value.placeLeft ? value.boxX + value.width : value.boxX) !== cursorX) { context.moveTo(cursorX, value.y); context.lineTo(value.placeLeft ? value.boxX + value.width : value.boxX, value.labelY); context.stroke(); }
  ...
  private createTimestampMarker(context: CanvasRenderingContext2D, cursorX: number, text: string, left: number, right: number, bottom: number, occupied: readonly HoverLabelRect[] = [], top = 0, emphasized = false): CanvasTimestampMarker {
    ...
    return { text, rect: layoutTimestampMarker(cursorX, width, 18, left, right, bottom, occupied, top), emphasized };
  }
  ...
  private hover(event: MouseEvent): void {
    ...
    const nextGraphIndex = graphIndexAtOffset(event.clientY - rect.top - 16, this.legendHeights); if (nextGraphIndex === undefined) { this.cursorTimestamp = undefined; this.hoverGraphIndex = undefined; this.scheduleInteractionDraw(); return; } this.hoverGraphIndex = nextGraphIndex;
  ...
  private updateMarkerControls(): void { (this.host.querySelector('.chart-marker-clear') as HTMLButtonElement).hidden = this.markers.size === 0; this.updateExportAction(); }
  ...
  private renderLegend(width: number): readonly number[] {
    ...
    for (let graphIndex = 0; graphIndex < graphs.length; graphIndex++) {
      ...
      graphs[graphIndex].forEach((group, axisIndex) => {
        for (const item of group.items) { const entry = document.createElement('span'); entry.className = 'chart-legend-entry'; const color = document.createElement('i'); color.style.background = this.color(item.definition.id); const axis = document.createElement('b'); axis.className = 'chart-axis-badge'; axis.textContent = axisIndex === 0 ? 'L' : 'R'; const label = document.createElement('span'); label.className = 'chart-legend-label'; label.textContent = signalLegendLabel(item.definition, this.mode, group.displayUnit); entry.append(color, axis, label); items.appendChild(entry); }
  ...
  private saveImage(): void {
    ...
    const chartTop = 58; this.paintChart(context, width, height, chartTop, height - chartTop - contentHeight, false, legendHeights);
    this.post({ type: 'saveTimeSeriesImage', dataUrl: canvas.toDataURL('image/png') });
  }
  ...
  private persist(): void { this.save({ chartSelectedIds: [...this.selected], chartColors: Object.fromEntries(this.colors), chartMode: this.mode, chartConnectGaps: this.connectGaps, chartAxisRange: this.axisRange, chartGroupLayout: this.groupLayout, chartPaneWidth: this.paneWidth, chartPaneCollapsed: this.paneCollapsed }); }
  private applyPane(): void { const toggle = this.host.querySelector('.pane-toggle')!; toggle.setAttribute('aria-expanded', String(!this.paneCollapsed)); toggle.setAttribute('title', this.paneCollapsed ? t('Show Signal selection', 'Signal選択を表示') : t('Hide Signal selection', 'Signal選択を隠す')); this.pane.style.width = this.paneCollapsed ? '0' : `${this.paneWidth}px`; this.pane.style.display = this.paneCollapsed ? 'none' : 'block'; this.separator.style.display = this.paneCollapsed ? 'none' : 'block'; }
