...
test('parses decimal, integer, and fraction resolution while retaining display text', () => {
  for (const [text, expected] of [['1', 1], ['0.0078125', 0.0078125], ['1/128', 1 / 128], ['3/10', 0.3], ['-0.01', -.01], ['-1/16', -1/16], ['1/-16', -1/16]] as const) {
...
test('rejects zero, zero denominator, non-finite, and configured magnitude violations', () => {
  for (const text of ['1/0', '0', '-0', 'Infinity', '1e-16', '1e16', '-1e-16', '-1e16']) assert.equal(parseResolution(text).valid, false, text);
});
...
test('fraction spinner uses ×2 for powers of two and ×10 otherwise', () => {
  ...
  assert.equal(resolutionStepFactor('-1/16'), 2);
  const negative = adjustResolution('-1/16', 'decrease'); assert.equal(negative.valid && negative.resolution.value, -1/32);
});
